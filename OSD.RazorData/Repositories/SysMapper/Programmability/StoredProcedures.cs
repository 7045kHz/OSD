...
namespace OSD.RazorData.Repositories.SysMapper.Programmability
{
    public class StoredProcedures : IStoredProcedures
    {
        ...
        public void ExecuteTx(string name)
        {
        ...
        public void ExecuteTx(string name, object param)
        {
            using (var transation = new TransactionScope())
            {
                try
        ...
        public async void ExecuteAsync(string name)
        {
            using (var cnn = new SqlConnection(ConnectionString))
            {
                await cnn.ExecuteAsync(name, commandType: CommandType.StoredProcedure);
            }
        }
        public async void ExecuteAsync(string name, object param)
        {
            using (var cnn = new SqlConnection(ConnectionString))
            {
                await cnn.ExecuteAsync(name, param, commandType: CommandType.StoredProcedure);
            }
        }
        ...
        public async Task<List<T>> ListAsync<T>(string name)
        {
            using (var cnn = new SqlConnection(ConnectionString))
            {
                var result = await cnn.QueryAsync<T>(name, commandType: CommandType.StoredProcedure);
